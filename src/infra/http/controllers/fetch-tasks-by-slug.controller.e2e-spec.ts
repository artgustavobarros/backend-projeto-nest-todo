...
describe('Fetch tasks by slug (E2E)', () => {
  ...
  test('[GET] /tasks/search/:slug', async () => {
    ...
    await Promise.all([
      taskFactory.makePrismaTask({
        authorId: user.id,
        slug: Slug.create('test-doesnt-match'),
      }),
      taskFactory.makePrismaTask({
        authorId: user.id,
        slug: Slug.create('test-matches'),
      }),
      taskFactory.makePrismaTask({
        authorId: user.id,
        slug: Slug.create('test-matches'),
    ...
    const response = await request(app.getHttpServer())
      .get('/tasks/search/matches')
      .set('Authorization', `Bearer ${accessToken}`)

    expect(response.statusCode).toBe(200)
    expect(response.body).toEqual({
      tasks: [
        expect.objectContaining({ slug: 'test-matches' }),
        expect.objectContaining({ slug: 'test-matches' }),
      ],
    })
  })
